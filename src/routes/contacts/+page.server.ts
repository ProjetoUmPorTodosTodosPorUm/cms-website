...
export const load: PageServerLoad = async ({ fetch, url, depends }) => {
	...
	return await listLoad('contact', fetch, url)
}

export const actions = {
	remove: async ({ fetch, request }) => {
		return await removeAction('contact', fetch, request)
	},

	restore: async ({ fetch, request }) => {
		return await restoreAction('contact', fetch, request)
	},

	hardRemove: async ({ fetch, request }) => {
		return await hardRemoveAction('contact', fetch, request)
	}
} satisfies Actions
