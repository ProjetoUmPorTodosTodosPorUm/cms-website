import type { PageServerLoad } from './$types'
import { listLoad, restoreAction, hardRemoveAction, removeAction } from '$utils'
import type { Actions } from './$types'

export const load: PageServerLoad = async ({ fetch, url, depends }) => {
	depends('app:list-load')

	return await listLoad('welcomed-family', fetch, url)
}

export const actions = {
	remove: async ({ fetch, request }) => {
		return await removeAction('welcomed-family', fetch, request)
	},

	restore: async ({ fetch, request }) => {
		return await restoreAction('welcomed-family', fetch, request)
	},

	hardRemove: async ({ fetch, request }) => {
		return await hardRemoveAction('welcomed-family', fetch, request)
	}
} satisfies Actions
