	...
	<AppContent {...appHeader} {isLoading} showActions={false} showRefreshButton={false}>
		<form class="app-form" method="POST" action="?/put" use:enhance={editSubmitFunction} on:submit={onSubmit}>
			<div class="input">
				...
				<input value={agendaData.title} name="title" placeholder={AGENDA_INPUT_LABELS.title} autocomplete="off" />
			</div>
			<div class="input input-lg">
				<TinyMCEEditor value={agendaData.message} formName="message" placeholder={AGENDA_INPUT_LABELS.message} />
				<!-- file={form?.apiData?.file} -->
			</div>
			<div class="input input-lg">
				<InputFiles files={initialFiles} formName="attachments" inputLabel={AGENDA_INPUT_LABELS.attachments} />
			</div>
			<div class="input">
				<Icon src={HiOutlineCalendar} />
				<input value={fromStringToISOLocale(agendaData.date)} name="date" type="datetime-local" autocomplete="off" />
			</div>
