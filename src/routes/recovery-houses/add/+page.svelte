<script lang="ts">
	...
	// App Header
	$: appHeader = {
		name: 'Adicionar Casa de Recuperação',
		buttonText: ''
	}
</script>
	...
	<AppContent {...appHeader} {isLoading} showActions={false} showRefreshButton={false} on:click={onSubmit}>
		<form class="app-form" method="POST" action="?/post" use:enhance on:submit={onSubmit}>
			<div class="input">
				...
				<input name="title" placeholder={RECOVERY_HOUSES_INPUT_LABELS.title} autocomplete="off" />
			</div>
			<div class="input input-lg">
				<TinyMCEEditor formName="description" placeholder={RECOVERY_HOUSES_INPUT_LABELS.description} />
				<!-- file={form?.apiData?.file} -->
			</div>
			<div class="input input-lg">
				<InputFile formName="image" accept="image/*" inputLabel={RECOVERY_HOUSES_INPUT_LABELS.image} />
			</div>
			<FieldSelect />
			<div class="input">
