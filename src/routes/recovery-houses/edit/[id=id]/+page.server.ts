...
export const load: PageServerLoad = async ({ fetch, cookies, params }) => {
	return await editLoad('recovery-house', fetch, cookies, params)
}

export const actions = {
	put: async ({ fetch, request, cookies, params }) => {
		const schema = yup.object().shape({
			title: yup.string().required(SHARED.yup.required(RECOVERY_HOUSES_INPUT_LABELS.title)),
			description: yup
				.string()
				.required(SHARED.yup.required(RECOVERY_HOUSES_INPUT_LABELS.description)),
		...
		try {
			...
			const res = await safeFetch(fetch, cookies, {
				url: `${PUBLIC_API_URL}/recovery-house/${params.id}`,
				method: 'PUT',
