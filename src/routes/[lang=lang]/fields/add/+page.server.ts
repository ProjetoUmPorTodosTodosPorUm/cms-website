...
export const actions = {
    post: async ({ fetch, request, locals, cookies }) => {
        ...
        const arrStreetRelation = streetRelation && streetRelation?.length > 0 ? streetRelation?.split(',') : []

        try {
            schema.validateSync({ continent, country, state, abbreviation, designation, mapLocation, mapArea, collectionPoints, streetRelation: arrStreetRelation }, { abortEarly: false })

            const res = await safeFetch(fetch, cookies, {
                url: `${PUBLIC_API_URL}/field`,
                method: 'POST',
                body: { continent, country, state, abbreviation, designation, mapLocation, mapArea, collectionPoints, streetRelation: arrStreetRelation }
            })
