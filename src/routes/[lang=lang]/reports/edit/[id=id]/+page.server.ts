...
import { generateMessages, safeFetch, actionErrorHandler, editLoad, bulkFileAction, fileRemoveAction } from '$utils'
...
export const actions = {
    put: async ({ fetch, request, locals, cookies, params }) => {
        ...
        try {
            schema.validateSync({ title, text, shortDescription, attachments: arrAttachments, month, year, type, field }, { abortEarly: false })

            const res = await safeFetch(fetch, cookies, {
                url: `${PUBLIC_API_URL}/report/${params.id}`,
                method: 'PUT',
                body: { title, text, shortDescription, attachments: arrAttachments, month, year, type, field }
            })
