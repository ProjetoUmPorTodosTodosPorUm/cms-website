<script lang="ts">
	...
	import type { AnnouncementDto, ColumnCell, Pagination, RowCell } from '$lib/types';
	import {
		delay,
		friendlyDateString,
	...
	let announcementData: AnnouncementDto[] = [];
	...
	async function loadData() {
		...
		try {
			...
			const res = await axios.get(`announcement?${queryString}`);
	...
	function handleEdit(event: CustomEvent) {
		...
		goto(`/${data.locale}/announcements/edit?id=${id}`);
	}
	...
	async function handleRemove(event: CustomEvent) {
		...
		if (remove) {
			...
			try {
				...
				announcementData = removeItemById(id, announcementData);
				isLoading = false;
			} catch (error) {
				isLoading = false;
